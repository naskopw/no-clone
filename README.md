...
> Local secret broker for AI agents — deliver credentials directly without
> exposing their values in agent context, command arguments, or shell history.

`no-clone` lets an AI agent run a command that needs credentials while the
human keeps control of those credentials. The agent asks for a secret by name,
such as `production/deploy-token`; the local broker resolves that name and
delivers the value directly to the target process.

The repository can therefore describe **which credentials a command needs**
without containing the credentials themselves. Secret values stay in an
encrypted local vault and are released only from profiles the human has
explicitly unlocked.

```text
Human                         no-clone                         Agent
  │                              │                              │
  ├─ stores secrets in vault ───▶│                              │
  ├─ unlocks a profile ─────────▶│◀── secret names + command ──┤
  │                              ├─ starts target process       │
  │                              └─ delivers secrets to target  │
  │                                                             │
  │                         Agent receives command output and exit status;
  │                         the target must not print delivered secrets
```

`no-clone` is early-stage software and has not had a full independent security
audit. Evaluate it carefully before using it with production credentials.

## Why use it?

AI-assisted command-line work often ends with a tool needing an API key,
deployment token, or database password. Common workarounds put that value in a
prompt, command argument, shell history, generated `.env` file, or the agent's
own environment.

With `no-clone`:

- the human enters and unlocks credentials through hidden prompts;
- the agent and repository refer only to profile and secret names;
- the broker injects each value into the target over `fd`, `stdin`, or `env`;
- unlocks expire automatically; and
- zero-trust sessions can require human authorization before every delivery.

This is a secret-delivery boundary, not a sandbox. The launched command receives
the credential and can still log, copy, or disclose it. `no-clone` does not
redact target output or protect against a malicious target command.
...
### 5. Agent or automation: run the command
...
agent integration guide in
[`skills/no-clone/SKILL.md`](skills/no-clone/SKILL.md) captures these operating
rules for compatible coding agents.
